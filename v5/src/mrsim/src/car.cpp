...
{   
    // 1) map collision using polygon hull
    ...
    // Build *world-space* hull of THIS car at candidate pose p
    const std::vector<Point> myHullW = carHullWorld(
        Pose(rear.x, rear.y, theta),
        L, d, wheel_length, wheel_width
    );

    // FreeFlying radius in meters = pixels * resolution
    auto ff_radius_m = [&](const FreeFlying* ff){
        return ff->radius_in_pixels * world->resolution;
    };

    for (WorldItem* it : world->items) {
        if (it == this) continue;

        if (auto* otherCar = dynamic_cast<CarRobot*>(it)) {
            // Build other car hull in world space (at its *current* pose)
            const float o_wlen = otherCar->L / 4.0f;
            const float o_wwid = otherCar->d / 4.0f;
            const std::vector<Point> otherHullW = carHullWorld(
                otherCar->pose, otherCar->L, otherCar->d, o_wlen, o_wwid
            );
            if (convexPolygonsIntersectSAT(myHullW, otherHullW))
                return true;

        } else if (auto* otherFF = dynamic_cast<FreeFlying*>(it)) {
            const float oR = ff_radius_m(otherFF);
            const Point  oC = otherFF->pose.translation();
            if (convexPolygonIntersectsCircle(myHullW, oC, oR))
                return true;
        }
    }
...
// bool CarRobot::collides(const Point &p)
// {
//     // ----- (A) map collision using polygon hull (your logic, unchanged) -----
//     float theta = pose.rotation();
//     Point rear = p;  // candidate rear-axle position (in meters)
//     Point dir(std::cos(theta), std::sin(theta));
//     Point front (rear.x + dir.x * L, rear.y + dir.y * L);
//     Point center((rear.x + front.x) / 2, (rear.y + front.y) / 2);
//     float cos_theta = std::cos(theta);
...
//         for (int c = bbox.x; c < bbox.x + bbox.width; ++c)
//         {
//             if (cv::pointPolygonTest(hull_px, cv::Point(c, r), false) >= 0)
//             {
//                 IndexPair ip(r, c);
...
//     // ----- (B) robot-vs-robot collision (circle approx. in meters) -----
//     const float eps = 1e-4f;

//     // circumscribed radius of our padded car rectangle
//     const float a   = 0.5f * L + wheel_length;  // half-length + pad
//     const float b   = d + wheel_width;          // half-width  + pad
//     const float myR = std::sqrt(a*a + b*b);
//     const Point  myC = center;
...
//             const Point  oC = otherFF->pose.translation();
//             const float sumR = myR + oR + eps;
//             if (dist2(myC, oC) < sumR * sumR) return true;

//         } else if (auto* otherCar = dynamic_cast<CarRobot*>(it)) {
//             // other car center and radius
//             const float o_wheel_len = otherCar->L * 0.25f;
//             const float o_wheel_wid = otherCar->d * 0.25f;
//             const float oa = 0.5f * otherCar->L + o_wheel_len;
//             const float ob = otherCar->d + o_wheel_wid;
//             const float oR = std::sqrt(oa*oa + ob*ob);

//             const float th = otherCar->pose.rotation();
//             const Point  dir(std::cos(th), std::sin(th));
//             const Point  oC(otherCar->pose.x + 0.5f * otherCar->L * dir.x,
//                             otherCar->pose.y + 0.5f * otherCar->L * dir.y);
